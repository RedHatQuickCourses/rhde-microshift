...
.. Allow remote access to the mirror registry on the system's firewall.
+
[source,subs="verbatim,quotes"]
--
...
+
We will install MicroShift with common assumptions of air-gapped environments, where MicroShift should only pull contrainer images that were previouslly vetted and only from private container registries, which in the classroom environment is the mirror registry..
...
.. Copy your registry authentication credentials to a pull secret file. It replaces the pull secret of your Red Hat account, that you would use to install OpenShift and MicroShift when connected to the internet.
+
[source,subs="verbatim,quotes"]
--
...
--
+
WARNING: You will later copy this file to your MicroShift instances, do it is important that it contains only credentials to the mirror registry, and nothing else. If you configured your mirror registry with exactly the same user name and password from the previous step, you can download a https://github.com/RedHatQuickCourses/rhde-build-samples/blob/main/microshift/mirror-pull-secret[sample pull secret file] from the course samples repository and just edit the host name of your mirror registry.

.. Download a file system copy of all images from the classroom server.
+
When outside of the classroom environment, follow the instructions from product docs to download MicroShift images to a staging directory. In the classroom environment, we provide you with a backup of the staging directory to avoid the need for internet access and save on bandwidth fees.
+
[source,subs="verbatim,quotes"]
--
...
--
+
NOTE: The previous command produces a bunch of `index.html*` files that you can safely ignore. We should have put a TAR backup in the classroom environment, instead of a directory tree.

3. Populate the mirror registry with container images required by MicroShift and sample applications for course activities.
...
+
In a production environment, it is recommended that you deploy Red Hat Satellite to mirror RPM package repositories from Red Hat products, but the classroom environment uses an Apache Web Server.
...
.. Open `http://content.example.com/rhde/rpms/` in a web browser. This directory contains a copy of the Red Hat OpenShift and Fast Data Path for RHEL repositories, created using the `reposync` command.
