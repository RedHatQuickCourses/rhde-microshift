...
+
Be patient, it will take a few moments for MicroShift to generate Kubernetes configuration files, pull all its containers, and starts it daemons and pods. Note that it will _not_ be ready yet when the `systemctl` command returns.>>>>>>> main
+
[source,subs="verbatim,quotes"]
--
