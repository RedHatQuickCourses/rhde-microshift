...
3. Download and inspect an Image Builder blueprint for a pre-configured MicroShift instance.
...
+
NOTE: The code snipets here help you locate the relevant sections in the kickstart file, but they do not list the entirery of the file. Follow along with a text editor and navigate through the kickstart file.
...
... Deploy the OSTree commit embeded in the installation media.
+
[source,subs="verbatim"]
--
...
--
+
NOTE: production systems would NOT enable password login.
...
.. Start a composer for your edge installer image.
+
[source,subs="verbatim,quotes,attributes"]
--
...
.. Disconnect from the VM console. by pressong kbd:[Ctrl+\]], and start a SSH session to your _edge machine_.
+
[source,subs="verbatim,quotes"]
--
...
--
+
NOTE: If you are a quick typer, you may get "no resources found" errors from `oc get pod`
...
You suceed provisioning an edge device, from an edge installer image, in an air-gapped environment: you only need the installation media to fully provision a ready-to-use RHEL for Edge system running a MicroShift instance.
