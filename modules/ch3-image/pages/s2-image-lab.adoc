...
* A _mirror registry machine_ with RHEL and unrestricted `sudo`, already populated with the container images required bu MicroShift and sample applications.
...
3. Download and inspect an Image Builder blueprint for a preconfigured MicroShift instance.
...
+
NOTE: The code snippets here help you locate the relevant sections in the blueprint, but they do not list the entirely of most sections. Follow along with a text editor and navigate through the blueprint file.
...
.. Push the blueprint and start a compose for an edge commit image.
+
...
5. On your _web server machine_, publish your new edge system image.
...
+
WARNING: If copy OSTree repostory files over another OSTree repository, you may get a corrupted repository.
+
[source,subs="verbatim,quotes"]
--
$ *ostree --repo=/var/www/html/repo refs*
...
6. On your _development machine_, download and inspect an Image Builder blueprint and a kickstart file for an edge installer image.
...
--
+
NOTE: production systems would NOT enable password login. We left it here as a convenience for the learner.
...
7. Build an edge installer image and download it as an ISO file.
...
8. Add your custom kickstart file to your installer ISO file.
...
9. On your _web server machine_, stop the mirror registry for Red Hat OpenShift, so you can prove that you can provision edge devices in air-gapped environments.
+
[source,subs="verbatim,quotes"]
--
...
10. On your _development machine_, create a KVM virtual machine to test your edge installer image.

.. Set a shell variable with the disk label of your installer image.
+
[source,subs="verbatim,quotes"]
--
...
+
NOTE:
You may need to press kbd:[Enter] to see the login prompt, after the VM stops displaying console messages.

.. Disconnect from the VM console. by pressing kbd:[Ctrl+\]], and start a SSH session to your _edge machine_.
+
[source,subs="verbatim,quotes"]
--
...
11. On your _edge machine_, verify that MicroShift is fully initialized.
...
--
+
NOTE: If you are too quick, you may get "no resources found" errors from `oc get pod`. If this happens, just wait a few seconds and try again.
...
You succeed provisioning an edge device, from an edge installer image, in an air-gapped environment: you only need the installation media to fully provision a ready-to-use RHEL for Edge system running a MicroShift instance.
