...
--
$ *systemtl is-active osbuild-composer.socket*
active
...
.. Check that the Image Buider service is configured with repository overrides for RHEL packages.
+
[source,subs="verbatim,quotes"]
--
...
--
+
NOTE: If Image Builder cannot access the container images referenced by the blueprint, it will show an error at the `composer-cli blueprints push` command.
...
.. Dettach from the VM console, by pressing kbd:[Ctrl+\]], and start a SSH session to your _edge machine_.
+
[source,subs="verbatim,quotes"]
--
